...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '315c1143-7878-44a1-ad0d-faf9e1f07d47'; // ID de Fantasy_salon
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Fantasy_salon';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '56652246';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'torresgonzalezamanda75@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Fantasy_salon!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'fantasy-salon';
};
...
console.log('✅ config-negocio.js listo para Fantasy_salon');
